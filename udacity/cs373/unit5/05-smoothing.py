...
def smooth(path, weight_data = 0.5, weight_smooth = 0.1, tolerance = 0.000001):
    ...
    while changed:
        ...
        for i in range(1, len(newpath) - 1):
            for j in range(0, 2):
                newpoint = newpath[i][j] + weight_data * (path[i][j] - newpath[i][j])
                newpoint = newpoint + weight_smooth * (newpath[i + 1][j] + newpath[i - 1][j] - 2 * newpoint)
                if abs(newpoint - newpath[i][j]) > tolerance:
                    changed = True
                newpath[i][j] = newpoint
